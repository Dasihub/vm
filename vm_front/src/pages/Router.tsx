...
import PassesPage from './PassesPage/PassesPage'
...
const Router: React.FC<{ access: null | number }> = ({ access }) => {
                ...
                <div className="main_container">
                    <div style={{ marginTop: '60px', borderRadius: '4px' }} className=" min-vh-100 w-100">
                        <Routes>
                            ...
                            />
                            <Route path="/token" element={<CheckTokenPage />} />
                            <Route path="/login" element={<Navigate replace to="/passes" />} />
                            <Route path="*" element={<Error404 />} />
                            <Route path="/" element={<Navigate replace to="/passes" />} />
                        </Routes>
                    </div>
                </div>
            )
        ...
        if (id_role == 1) {
            return (
                <div className="main_container">
                    <div style={{ marginTop: '110px', borderRadius: '4px' }} className=" min-vh-100 w-100">
                        <Routes>
                            <Route path="/dekanat" element={access == 1 ? <MainPage /> : <Navigate replace to="/teacher" />} />
                            <Route
                                path="/teacher"
                                element={
                                    <React.Suspense fallback={Load}>
    ...
    return (
        <Routes>
            ...
            <Route path="/passes" element={<Navigate replace to="/login" />} />
